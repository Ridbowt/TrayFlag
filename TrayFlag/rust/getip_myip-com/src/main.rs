...
// ← For parsing the response from myip.com (only ip + cc)
#[derive(Deserialize)]
struct MyIpResponse {
    ip: String,
    cc: String,
...
fn fetch_myip() -> Result<MyIpResponse, String> {
...
fn main() {
    ...
    let result = invoke_with_retry(fetch_myip, 3, 1);

    let output = match result {
        Ok(data) => {
            // Success: populate full_data
            // city and isp = "N/A" (as in original)
            Output {
                ip: data.ip.clone(),
                full_data: FullData {
                    ip: data.ip.clone(),
