...
def get_base_path():
    ...
    if getattr(sys, 'frozen', False):
        ...
        return os.path.dirname(os.path.abspath(__file__)) # <--- ИЗМЕНЕНО ЗДЕСЬ
...
# --- НОВАЯ ФУНКЦИЯ ДЛЯ ОБЕСПЕЧЕНИЯ СУЩЕСТВОВАНИЯ INI-ФАЙЛА ---
def ensure_ini_file_exists(file_path):
    ...
    if not os.path.exists(file_path):
        try:
            # Создаем пустой файл или с минимальным содержимым
            # QSettings сам запишет туда данные при первом сохранении.
            # Но наличие файла может помочь ему правильно его найти.
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(f"[{APP_NAME}]\n") # Добавим хотя бы секцию по умолчанию
                f.write("version = 1.0\n") # И какой-нибудь параметр
            print(f"Created new INI file at: {file_path}")
        except Exception as e:
            print(f"ERROR: Could not create INI file at {file_path}: {e}")
            # Если не удалось создать, QSettings может попытаться создать его в другом месте
            # или работать с настройками в памяти.

# =============================================================================
#  Остальные функции и классы вашего приложения
# =============================================================================

def get_location_data():
    """
    Получает данные о местоположении, пробуя несколько сервисов по очереди.
    """
    services = [
        "https://ip-api.com/json/?fields=status,message,countryCode,city,isp,query",
    ...
    for url in services:
        try:
            ...
            print(f"Warning: Failed to connect to {url}. Trying next service. Error: {e}")
            continue

    raise ConnectionError("All IP services are unavailable.")
...
def set_autostart_shortcut(enabled):
    ...
    if enabled:
        # Ярлык должен указывать на главный .exe
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(self, app_icon, tr, available_langs, current_lang, parent=None):
        ...
        # --- Вызов новой функции перед инициализацией QSettings ---
...
if __name__ == "__main__":
    ...
    # --- ДОБАВЛЕНО: Установка имени приложения и организации для QSettings ---
