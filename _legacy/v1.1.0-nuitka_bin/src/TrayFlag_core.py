# src/TrayFlag_core.py

# =============================================================================
#  ЭТАП 1: ИМПОРТЫ
#  Все импорты происходят сразу, bootstrap не нужен.
# =============================================================================
import sys
import os
import requests
...
# Проверка доступности библиотек для звука
try:
...
def get_base_path():
    """
    Возвращает базовый путь к корню дистрибутива (папке 'dist').
    """
    if getattr(sys, 'frozen', False):
        # Наш .exe ('TrayFlag_core.exe') лежит в 'bin', поэтому нам нужно 
        # подняться на один уровень вверх ('..') чтобы найти 'assets' и место для '.ini'.
        return os.path.abspath(os.path.join(os.path.dirname(sys.executable), '..'))
    else:
        # Для обычного запуска .py из 'src'
        return os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))

def resource_path(relative_path):
    """
    Возвращает полный путь к файлу ресурсов (например, в папке assets).
    """
...
def set_autostart_shortcut(enabled):
    ...
    # Ярлык должен указывать на главный .exe, а не на тот, что в bin
    shortcut_path = os.path.join(startup_folder, f"{APP_NAME}.lnk")
    
    if enabled:
        # Путь к главному .exe в корне dist
        target_path = os.path.join(get_base_path(), f"{APP_NAME}.exe")
        ...
        # Рабочая директория - корень dist
...
class TrayFlag(QtWidgets.QSystemTrayIcon):
    ...
    def play_notification_sound_threaded(self):
        if not self.sound_enabled or self.sound_samples is None or not sound_libs_available: return
